...
| --- | --- | --- | --- |
| %IncMSE | YES (if importance = TRUE) | NO | $\checkmark$ |
| IncNodePurity | YES | NO  |  |
| MeanDecreaseGini | NO | YES |  |
| MeanDecreaseAccuracy | NO | YES (if importance = TRUE) | $\checkmark$ |


If we ignore the `type` argument of the `importance()` function:

- Regression: if you do not specify `importance=TRUE` in the call to `randomForest()`, only `IncNodePurity` is output. Otherwise `%IncMSE` is output in column 1, and `IncNodePurity` is output in column 2.

- Classification: if you do not specify `importance=TRUE` in the call to `randomForest()`, only `MeanDecreaseGini` is output. Otherwise $p+2$ columns are output, where $p$ is the number of levels for the response variable; `MeanDecreaseAccuracy` is output in column $p+1$ and `MeanDecreaseGini` is output in column $p+2$.
...
1. Grow forest. Using "out-of-bag" (or OOB) data, compute the MSE. Call this MSE.min. (Out-of-bag data are the data that are left out of tree building, for each tree in the forest.)
