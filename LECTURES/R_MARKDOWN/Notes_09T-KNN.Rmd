...
`knn()` will output class predictions (based on majority vote!). Working with the output so as to, e.g., make a ROC curve for KNN is tricky but doable. If this is something you would like to do for your (classification-based) semester project, let me know and I can send you some code that turns `knn()` output into a Class 1 probability.
